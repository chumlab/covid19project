...
def print_geonameid_completeness(df):
    ...
    """
    all_tweets = df[~df['tweet_user_location_copy'].isin(
        LOCATION_DISCARD)]['tweet_id'].sum()
...
def show_nan(df):
    ...
    """
    nan_df = df[(~df['tweet_user_location_copy'].isin(
        LOCATION_DISCARD)) & df['geonameid'].isnull()]
