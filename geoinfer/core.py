        ...
        if len(admin1) > 1:
            ...
            if len(element) == 2:
                ...
                if len(admin1) == 1:
                    ...
                    admin1 = admin1_df[admin1_df['code'].str.contains(
                        rf'.{element}$')].copy()
...
def geonameid_from_location(location, cities_df, admin1_df, countries_df):
    # Datasets
    # * countries_df
    # * admin1_df
    # * cities_df
    ...
    # Split on comma (,)
    # The algo. starts with the last element and makes its way up to infer
    # (e.g. "Los Angeles, CA" -> ["Los Angeles", "CA"])
    ...
    # Two items (0, 1)
        ...
        if country is not None:
            ...
            admin1 = get_admin1(
                admin1_df, elements[0], country_code=country_code)
        ...
        if admin1 is not None:
            ...
            country_code, admin1_code = tuple(
                str(admin1['code'].item()).split('.'))
    ...
    # Three items
        ...
        if country is not None:
            ...
            # Get admin1 (restrict to <country>)
            admin1 = get_admin1(
                admin1_df, elements[1], country_code=country_code)
            ...
            if admin1 is not None:
                ...
                # Get city (restrict to <state/province>)
                city = get_city(
                    cities_df, elements[0], admin1_code=admin1_code)
